...
# ══════════════════════════════════════════════
# SECTION 45 — HTTP Request Smuggling
# ══════════════════════════════════════════════
def test_request_smuggling(host, port):
    section("45 · HTTP Request Smuggling (CL + TE conflict)")
...
# ══════════════════════════════════════════════
# SECTION 46 — CRLF Injection
# ══════════════════════════════════════════════
def test_crlf_injection(host, port):
    section("46 · CRLF injection in headers")
...
# ══════════════════════════════════════════════
# SECTION 47 — Host header injection
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
# SECTION 48 — Absolute URI in request line
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
# SECTION 49 — Fragment in URI
# ══════════════════════════════════════════════
def test_uri_fragment(host, port):
    section("49 · URI with fragment identifier")
...
# ══════════════════════════════════════════════
# SECTION 50 — Deeply nested path
# ══════════════════════════════════════════════
def test_deep_path(host, port):
    section("50 · Excessively nested URI path")
...
# ══════════════════════════════════════════════
# SECTION 51 — Zero Content-Length with body
# ══════════════════════════════════════════════
def test_zero_cl_with_body(host, port):
    section("51 · Content-Length: 0 but body present")
...
# ══════════════════════════════════════════════
# SECTION 52 — Negative Content-Length
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
# SECTION 53 — Overflow Content-Length
# ══════════════════════════════════════════════
def test_overflow_content_length(host, port):
    section("53 · Overflow Content-Length value")
...
# ══════════════════════════════════════════════
# SECTION 54 — Repeated headers
# ══════════════════════════════════════════════
def test_repeated_headers(host, port):
    section("54 · Repeated header names")
...
# ══════════════════════════════════════════════
# SECTION 55 — Extra spaces in request line
# ══════════════════════════════════════════════
def test_request_line_spaces(host, port):
    section("55 · Extra spaces in request line")

    # Double space between method and URI — lenient servers may accept
    raw = f"GET  / HTTP/1.1\r\nHost: {host}:{port}\r\nConnection: close\r\n\r\n".encode()
    resp = raw_request(host, port, raw, timeout=5)
    ...
    # Leading spaces before method — RFC 7230 §3.5 says MUST NOT ignore
    raw3 = f"  GET / HTTP/1.1\r\nHost: {host}:{port}\r\nConnection: close\r\n\r\n".encode()
...
# ══════════════════════════════════════════════
# SECTION 56 — Tab in header
# ══════════════════════════════════════════════
def test_tab_in_header(host, port):
    section("56 · Tab character in header value")
...
# ══════════════════════════════════════════════
# SECTION 57 — Slow Loris
# ══════════════════════════════════════════════
def test_slow_loris(host, port):
    section("57 · Slow Loris — incomplete request never finishes")
...
# ══════════════════════════════════════════════
# SECTION 58 — CGI env injection via query string
# Tests that %0a in query string is sanitised and
# cannot inject extra env vars into the CGI process
# ══════════════════════════════════════════════
def test_cgi_env_injection(host, port):
    section("58 · CGI environment variable injection")
...
# ══════════════════════════════════════════════
# SECTION 59 — POST binary body with null bytes
# ══════════════════════════════════════════════
def test_binary_null_bytes(host, port):
    section("59 · POST binary body containing null bytes")
